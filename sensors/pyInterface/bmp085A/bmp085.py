...
# Constructor
def __init__(self, address=0x77, mode=1, bus_nr=1, debug=True):
...
def readCalibrationData(self):
	...
	self._cal_AC1 = self.int_sw_swap(self.addr,self.__BMP085_CAL_AC1)   # INT16
	self._cal_AC2 = self.int_sw_swap(self.addr,self.__BMP085_CAL_AC2)   # INT16
	self._cal_AC3 = self.int_sw_swap(self.addr,self.__BMP085_CAL_AC3)   # INT16
	self._cal_AC4 = self.int_sw_swap(self.addr,self.__BMP085_CAL_AC4)   # UINT16
	self._cal_AC5 = self.int_sw_swap(self.addr,self.__BMP085_CAL_AC5)   # UINT16
	self._cal_AC6 = self.int_sw_swap(self.addr,self.__BMP085_CAL_AC6)   # UINT16
	self._cal_B1 = self.int_sw_swap(self.addr,self.__BMP085_CAL_B1)     # INT16
	self._cal_B2 = self.int_sw_swap(self.addr,self.__BMP085_CAL_B2)     # INT16
	self._cal_MB = self.int_sw_swap(self.addr,self.__BMP085_CAL_MB)     # INT16
	self._cal_MC = self.int_sw_swap(self.addr,self.__BMP085_CAL_MC)     # INT16
	self._cal_MD = self.int_sw_swap(self.addr,self.__BMP085_CAL_MD)     # INT16
	if (self.debug):
...
def readRawTemp(self):
    ...
    self.bus.write_byte_data(self.addr, self.__BMP085_CONTROL, self.__BMP085_READTEMP)
    self.i2c.write8(self.__BMP085_CONTROL, self.__BMP085_READTEMPCMD)
    time.sleep(0.005)  # Wait 5ms
    raw = self.int_sw_swap(self.addr,self.__BMP085_TEMPDATA)
...
def readRawPressure(self):
	...
	self.bus.write_byte_data(self.addr,self.__BMP085_CONTROL, self.__BMP085_READPRESSURECMD + (self.mode << 6))
	...
	#self.i2c.write8(self.__BMP085_CONTROL, self.__BMP085_READPRESSURECMD + (self.mode << 6))
    	if (self.mode == self.__BMP085_ULTRALOWPOWER):
    		...
    		time.sleep(0.008)
		msb = self.bus.read_byte_data(self.addr,self.__BMP085_PRESSUREDATA)
		lsb = self.bus.read_byte_data(self.addr,self.__BMP085_PRESSUREDATA+1)
		msb = self.bus.read_byte_data(self.addr,self.__BMP085_PRESSUREDATA +2)
