...
class bmp085 :
  ...
  def readCalibrationData(self):
	...
	self._cal_AC1 = self.int_sw_swap(self.__BMP085_CAL_AC1)   # INT16
	self._cal_AC2 = self.int_sw_swap(self.__BMP085_CAL_AC2)   # INT16
	self._cal_AC3 = self.int_sw_swap(self.__BMP085_CAL_AC3)   # INT16
	self._cal_AC4 = self.int_sw_swap(self.__BMP085_CAL_AC4)   # UINT16
	self._cal_AC5 = self.int_sw_swap(self.__BMP085_CAL_AC5)   # UINT16
	self._cal_AC6 = self.int_sw_swap(self.__BMP085_CAL_AC6)   # UINT16
	self._cal_B1 = self.int_sw_swap(self.__BMP085_CAL_B1)     # INT16
	self._cal_B2 = self.int_sw_swap(self.__BMP085_CAL_B2)     # INT16
	self._cal_MB = self.int_sw_swap(self.__BMP085_CAL_MB)     # INT16
	self._cal_MC = self.int_sw_swap(self.__BMP085_CAL_MC)     # INT16
	self._cal_MD = self.int_sw_swap(self.__BMP085_CAL_MD)     # INT16
	if (self.debug):
  ...
  def showCalibrationData(self):
	#     "Displays the calibration values for debugging purposes"
      ...
      print "DBG: MC  = %6d" % elf._cal_MC)
      print "DBG: MD  = %6d" % (self._cal_MD)

  def readRawTemp(self):
    #"Reads the raw (uncompensated) temperature from the sensor"
	self.bus.write_byte_data(self.addr, self.__BMP085_CONTROL, self.__BMP085_READTEMPCMD)
 #   self.i2c.write8(self.__BMP085_CONTROL, self.__BMP085_READTEMPCMD)
	time.sleep(0.005)  # Wait 5ms
	raw = self.int_sw_swap(self.__BMP085_TEMPDATA)
	#raw = self.readU16(self.__BMP085_TEMPDATA)
	if (self.debug):
		print "DBG:Raw Tp: 0x%04X (%d)" % (raw & 0xFFFF, raw)
    return raw
  ...
  def readRawPressure(self):
	...
	xlsb = self.bus.read_byte_data(self.addr,self.__BMP085_PRESSUREDATA +2)
